...
const CTASection: React.FC<CTASectionProps> = ({ cta }) => {
  const handleButtonClick = () => {
    // 전체 상품 페이지로 이동 또는 다른 액션
    console.log('전체 상품 보기 클릭');
    // navigate('/products');
  };
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center">
        ...
        <button
          ...
          className="mb-12 flex items-center gap-4 rounded-full bg-[#0d9dda] px-12 py-5 font-bold text-white shadow-[0px_0px_15px_0px_rgba(13,157,218,0.4)] transition-colors hover:bg-[#0b8bc4]"
        >
          <span className="text-xl leading-7">{cta.buttonText}</span>
