...
const CheckboxFilter = ({ label, options, selectedOptions, onToggle }: CheckboxFilterProps) => {
  return (
    <div className="flex items-start gap-4 py-2">
      <h3 className="min-w-[80px] py-1 pr-6 text-[13px] font-semibold tracking-tight text-[#1d1d1f]">
        {label}
      </h3>
          ...
          <label key={option.value} className="group flex cursor-pointer items-center gap-2.5">
            <div className="relative flex h-4 w-4 items-center justify-center">
              <input
                ...
                className="peer h-4 w-4 cursor-pointer appearance-none rounded-[4px] border border-[#d2d2d7] bg-white transition-all checked:border-[#1d1d1f] checked:bg-[#1d1d1f]"
              />
              <svg
